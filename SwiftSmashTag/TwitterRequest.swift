    ...
    func performTwitterRequest(request: SLRequest, handler: (PropertyList?) -> Void) {
        if let account = twitterAccount {
            ...
            request.performRequestWithHandler { (jsonResponse, httpResponse, _) in
                //
                // taken from http://stackoverflow.com/questions/31073497/swift-extra-argument-error-in-call
                do {
                    if let propertyListResponse = try NSJSONSerialization.JSONObjectWithData(jsonResponse, options: NSJSONReadingOptions.MutableLeaves) as? NSDictionary {
                        self.synchronize {
